...
export function render(container) {
                ...
                <div class="bg-white/90 backdrop-blur-sm border border-slate-300 rounded-3xl p-5 shadow-sm space-y-4">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-1">
                            <button id="wf-dose1-dec" class="w-10 h-12 bg-slate-100 hover:bg-slate-200 border border-slate-300 rounded-xl font-bold text-slate-700 flex items-center justify-center transition-all cursor-pointer text-lg">-</button>
                            <input type="number" id="wf-dose-old" step="0.1" placeholder="0" class="flex-1 bg-white border border-slate-300 rounded-xl px-3 py-2 text-center font-extrabold text-slate-800 text-2xl h-12 focus:outline-none focus:border-teal-500">
                            <button id="wf-dose1-inc" class="w-10 h-12 bg-slate-100 hover:bg-slate-200 border border-slate-300 rounded-xl font-bold text-slate-700 flex items-center justify-center transition-all cursor-pointer text-lg">+</button>
                        </div>
                    </div>
                ...
                <div class="bg-white/90 backdrop-blur-sm border border-slate-300 rounded-3xl p-5 shadow-sm space-y-4">
                    ...
                    <div>
                        ...
                        <div class="flex items-center gap-2">
                            <button id="wf-dose2-dec" class="w-10 h-12 bg-slate-100 hover:bg-slate-200 border border-slate-300 rounded-xl font-bold text-slate-700 transition-all cursor-pointer text-lg">-</button>
                            <input type="number" id="wf-dose-new" step="0.5" placeholder="0" class="flex-1 bg-white border border-slate-300 rounded-xl px-3 h-12 text-center font-extrabold text-slate-800 text-2xl focus:outline-none focus:border-teal-500">
                            <button id="wf-dose2-inc" class="w-10 h-12 bg-slate-100 hover:bg-slate-200 border border-slate-300 rounded-xl font-bold text-slate-700 transition-all cursor-pointer text-lg">+</button>
    ...
    // Key Rules Config (ข้อ 5: ปรับปรุง Boundary ให้ชัดเจน ไม่ซ้ำซ้อน)
    const RULES_NORMAL = [
        ...
        { minINR: 3.0, maxINR: 4.0, isStrictMax: false, inrLabel: '> 3.0 - < 4.0', sug: 'Decrease 5-10%', lowMult: 0.9, highMult: 0.95, pctLow: -10, pctHigh: -5 },
    ...
    const RULES_HIGH = [
        ...
        { minINR: 3.5, maxINR: 4.0, isStrictMax: false, inrLabel: '> 3.5 - < 4.0', sug: 'Decrease 5-10%', lowMult: 0.9, highMult: 0.95, pctLow: -10, pctHigh: -5 },
    ...
    function updateInput2Logic(currentMatchedRule) {
        ...
        if (Math.abs(pct) < 0.001) {
            ...
            const sign = pct > 0 ? '+' : '';
            // ข้อ 4: เอาเครื่องหมาย < > ออก เหลือแค่ตัวเลขและเครื่องหมาย +/-
    ...
    // 3. Tablet Render Helper (ข้อ 6.1, 6.2, 6.3: วาดเม็ดยาแบบโล้นๆ ไม่ใส่ตัวเลข + ทำครึ่งเม็ดไล่สีขาว)
    function renderTabletUI(mg) {
        ...
        if (mg === 2) { colorBg = 'bg-orange-500'; }
        else if (mg === 1) { colorBg = 'bg-orange-500'; isHalf = true; }
        else if (mg === 3) { colorBg = 'bg-sky-500'; }
        else if (mg === 1.5) { colorBg = 'bg-sky-500'; isHalf = true; }
        else if (mg === 5) { colorBg = 'bg-pink-500'; }
        else if (mg === 2.5) { colorBg = 'bg-pink-500'; isHalf = true; }

        // ข้อ 6.1: กรณีครึ่งเม็ด ทำพื้นหลังครึ่งวงกลมไล่สีขาว
        if (isHalf) {
            return `
                <div class="w-7 h-7 rounded-full border border-slate-300 shadow-2xs" 
                     style="background: linear-gradient(90deg, ${mg === 1 ? '#f97316' : mg === 1.5 ? '#0ea5e9' : '#ec4899'} 50%, #ffffff 50%);">
                </div>
            `;
        }

        // ข้อ 6.2: เม็ดเต็ม โล้นๆ ไม่มีตัวเลขในวงกลม
        return `
            <div class="w-7 h-7 rounded-full ${colorBg} shadow-2xs"></div>
        `;
    }
    ...
    function renderDayCell(dailyDoseCombo) {
        ...
        return `
            <div class="p-1.5 flex flex-col items-center justify-between min-h-[64px] bg-white rounded-xl border border-slate-100 shadow-2xs">
                ...
                <!-- ข้อ 6.3: ระบุปริมาณยารวมใต้เม็ดยา -->
    ...
    function optimizeDailyItems(items, has5mg) {
        ...
        // 7.1 ยุบ 2mg + 3mg -> 5mg (เต็มเม็ด)
        if (optimized.includes(2) && optimized.includes(3)) {
            optimized = optimized.filter(x => x !== 2 && x !== 3);
            optimized.push(5);
        }

        // 7.2 ยุบ ครึ่งเม็ด 2mg (1mg) + ครึ่งเม็ด 3mg (1.5mg) -> ครึ่งเม็ด 5mg (2.5mg)
    ...
    function generateSchedulePatterns(targetWeeklyDose) {
        ...
        // Optimize Items ตาม Smart Suggestion
                    ...
                    <table class="w-full text-center text-xs border-collapse">
                        <thead>
                            <tr class="text-[11px] font-bold text-slate-600 border-b border-slate-200">
                                <!-- ข้อ 8: คอลัมน์ Dose/wk ลดความกว้าง 10% + จัดกลาง -->
    ...
    // Event Listeners & Step Logic (ข้อ 9: กด Enter แล้วข้ามไป Input ถัดไป)
    ...
    // Reset Buttons (ข้อ 2: เปลี่ยนเป็นปุ่มสีม่วงอ่อน)
